# This model is trained on a combined dataset of original and adversarial samples.
import os
...
class AdversarialDataset(Dataset):
    ...
    def __getitem__(self, idx):
        img_name = os.path.join(self.img_dir, os.path.basename(self.data.iloc[idx, 0]))  # First column: Image Path
        true_label = int(self.data.iloc[idx, 1])  # Second column: True Label
...
def setup(params):
    ...
    train_adv_dataset = AdversarialDataset(
        csv_file="./adversarial_dataset/train/train_adv_samples.csv", 
        img_dir="./adversarial_dataset/train", 
        transform=transform
    )
    val_adv_dataset = AdversarialDataset(
        csv_file="./adversarial_dataset/validation/valid_adv_samples.csv", 
        img_dir="./adversarial_dataset/validation", 
        transform=transform
    )
    test_adv_dataset = AdversarialDataset(
        csv_file="./adversarial_dataset/test/test_adv_samples.csv", 
        img_dir="./adversarial_dataset/test", 
        transform=transform
    )
    
    #Loading original MNIST datasets
    ...
    train_dataset = ConcatDataset([train_dataset, train_adv_dataset])
    val_dataset = ConcatDataset([val_dataset, val_adv_dataset])
    test_dataset = ConcatDataset([test_dataset, test_adv_dataset])
...
def run(params):
    ...
    for epoch in range(params.get("epochs")):
        ...
        epoch_test_acc.append(test_accuracy / len(test_loader.dataset))

        print(f"Epoch: {epoch} | Train Loss: {epoch_train_loss[-1]:.4f} | Train Acc: {epoch_train_acc[-1]:.4f} || Val Loss: {epoch_val_loss[-1]:.4f} | Val Acc: {epoch_val_acc[-1]:.4f} || Test Acc: {epoch_test_acc[-1]:.4f}")
    ...
    plt.plot([i+1 for i in range(params.get("epochs"))], epoch_test_acc, label='Testing')
    plt.legend()
